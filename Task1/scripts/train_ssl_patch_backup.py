...
def debug_ssl_batch(train_loader, model, device, max_print_samples=3):
    ...
    model.eval()
    with torch.no_grad():
        z_patches, patch_indices = model(v1.to(device))

        print("local embedding shape:", tuple(z_patches.shape))
        print("patch_indices shape:", tuple(patch_indices.shape))

        z_flat = z_patches.reshape(-1, z_patches.shape[-1])
        sim = torch.matmul(z_flat, z_flat.T)

        print(
            f"embedding mean/std: {z_flat.mean().item():.6f}, "
            f"{z_flat.std().item():.6f}"
        )
...
def objective(trial, base_args):
    ...
    # 每个 trial 单独目录
    args.output_dir = os.path.join(base_args.output_dir, f"optuna_trial_{trial.number}")
